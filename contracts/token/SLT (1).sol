...
//下面开始业务逻辑
    ...
    function buyV1( ) external checkBlackList {
    ...
    function claimDivdends() external checkBlackList {
        ...
        if (userInfo[msg.sender].refer_n >=3) {
            tempDividends += totalDividends[currentDivdendsRound] * 3 /10 / threeRefersNum; //30%⽤于给有效直推⼤于=⼈的⽤户进⾏奖励⽠分
        }
        if (userInfo[msg.sender].refer_n >=5) {
            tempDividends += totalDividends[currentDivdendsRound] * 4 /10 / fiveRefersNum; //40%⽤于给有效直推⼤于=5⼈的⽤户进⾏奖励⽠分
        }
        if (userInfo[msg.sender].refer_n >=10) {
            tempDividends += totalDividends[currentDivdendsRound] * 3 /10 / tenRefersNum; //30%⽤于给有效直推⼤于=10⼈的⽤户进⾏奖励⽠分
        }
